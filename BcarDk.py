...
mydb = sql.connect(host='localhost', user='dinesh4m90', password='Dinesh4Mysql@1990',
                                     port='3306', database='bizcardx_db')
...
with st.sidebar:
    selected = option_menu("Main Menu", ["Upload & Extract", 'Modify'], 
        icons=['upload', 'settings'], menu_icon="cast", default_index=1)
    selected
...
if selected == "Upload & Extract":
    ...
    if uploaded_card is not None:
        ...
        def save_card(uploaded_card):
            with open(os.path.join(r"C:\Users\DineshMohan\Desktop\python38\Demo_Files", uploaded_card.name), "wb") as f:
        ...
        extracted_text = reader.readtext(os.path.join(r"C:\Users\DineshMohan\Desktop\python38\Demo_Files", uploaded_card.name), detail=0)
        content=[]
        ...
        if st.button("Save Data"):
            ...
            try:
                with open(os.path.join(r"C:\Users\DineshMohan\Desktop\python38\Demo_Files", uploaded_card.name), "rb") as f:
                    image = f.read()
...
if selected == "Modify":
    ...
    with col1:
        ...
        # Check if DataFrame is not empty before using item()
        if not data_frame.empty:
            company_name = st.text_input("Enter the correct Company Name", data_frame.loc[data_frame["ID"] == id, "Company Name"].item())
            card_holder = st.text_input("Enter the correct Card Holder Name", data_frame.loc[data_frame["ID"] == id, "Card Holder"].item())
            designation = st.text_input("Enter the correct Designation", data_frame.loc[data_frame["ID"] == id, "Designation"].item())
            mobile_number = st.text_input("Enter the correct Mobile Number", data_frame.loc[data_frame["ID"] == id, "Mobile Number"].item())
            email = st.text_input("Enter the correct Email", data_frame.loc[data_frame["ID"] == id, "Email"].item())
            website = st.text_input("Enter the correct Website URL", data_frame.loc[data_frame["ID"] == id, "Website"].item())
        else:
            st.warning("Data frame is empty.")
    with col2:
        area = st.text_input("Enter the correct Area", data_frame.loc[data_frame["ID"] == id, "Area"].item())
        city = st.text_input("Enter the correct City", data_frame.loc[data_frame["ID"] == id, "City"].item())
        state = st.text_input("Enter the correct State", data_frame.loc[data_frame["ID"] == id, "State"].item())
        pin_code = st.text_input("Enter the correct Pin Code", data_frame.loc[data_frame["ID"] == id, "Pin Code"].item())
